...
struct Tensor {
    ...
    Tensor sigmoid_tensor(const Tensor& x) {
        ...
        for (size_t i = 0; i < x.data.size(); i++) {
            out.data[i] = 1.0f / (1.0f + exp(-x.data[i]));
        }

        return out;
    }

    Tensor sigmoid_derivative_tensor(const Tensor& x) {
        Tensor out(x.rows, x.cols);

        for (size_t i = 0; i < x.data.size(); i++) {
            float s = 1.0f / (1.0f + exp(-x.data[i]));
            out.data[i] = s * (1.0f - s);
        }
    ...
    Tensor loss_derivative(const Tensor& aL, const Tensor& y) {
        Tensor out(aL.rows, aL.cols);

        for (size_t i = 0; i < aL.data.size(); i++) {
            out.data[i] = 2.0f * (aL.data[i] - y.data[i]);
        }
    ...
    Tensor compute_weight_gradient(
    const Tensor& a_prev,
    const Tensor& zL,
    const Tensor& aL,
    const Tensor& y
    ) {
        Tensor t1 = a_prev;                         // a(L-1)
        Tensor t2 = sigmoid_derivative_tensor(zL); // sigmoid'(zL)
        Tensor t3 = loss_derivative(aL, y);        // 2(aL - y)

        Tensor grad(aL.rows, aL.cols);

        for (size_t i = 0; i < grad.data.size(); i++) {
            grad.data[i] = t1.data[i] * t2.data[i] * t3.data[i];
        }
...
// Derivative of sigmoid (using x directly)
